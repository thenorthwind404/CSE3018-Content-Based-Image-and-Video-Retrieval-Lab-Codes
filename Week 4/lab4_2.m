...
query_image = rgb2hsv(query_image);
...
[Qbinval_red,Qbinloc_red] = imhist(q_red);
...
names = {'file_name'};
for i=0:255
names{end+1} = sprintf('%s%d', 'Red Color Bin ', i);
end
for i=0:255
...
info_table = cell2table(cell(0, 770), 'VariableNames',names);
...
for k = 1:numel(S)
    ...
    [binval_red,binloc_red] = imhist(red);
...
%disp(info_table);
writetable(info_table, 'lab4_1.xlsx','Sheet',2)
